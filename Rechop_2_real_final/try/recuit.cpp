...
int pop(antenne& Antenne,int j){
    ...
    if (j+1 < Antenne.size_antennes){
        for(int i =j;i<Antenne.size_antennes;i++){
...
InitRandom();
solution xbfinal(Parser,Cluster,int(Cluster.nombre_de_point/6)+1);
xbfinal.first_solution(Parser,Cluster);

cout << xbfinal.cout_non_incremental(Parser,Cluster) <<"succces" <<endl;
...
for (int bouclesize = int(Cluster.nombre_de_point/6)+1;bouclesize<min(30,Cluster.nombre_de_point)+1;bouclesize++){
        ...
        for (int compteur =0; compteur<taille;compteur++){
            ...
            while (a) {
                ...
                if (p<proba1*proba1){
                    ...
                    else if(xprime.Antenne[j].size_antennes == 5){
                        a = true;
                    }

                    else{
                        int i1 = hasard(0,xprime.Antenne[i].size_antennes-1);
                        int j1 = hasard(0,xprime.Antenne[j].size_antennes);
                        // on enlève le i1 élément de la première chaine et on le rajoute dans j

                        push(xprime.Antenne[j],pop(xprime.Antenne[i],i1),j1);
                        a = false;
                    }
                }
    ...
    if (xb.cout_non_incremental(Parser,Cluster)<xbfinal.cout_non_incremental(Parser,Cluster)){
        xb.copy(xbfinal);
        cout << xbfinal.cout_non_incremental(Parser,Cluster)<<"on avance bien" << endl;
    }
}
return xbfinal;
}
